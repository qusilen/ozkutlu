...
const DetailClient = ({product}: {product: any}) => {
  ...
  let productRating = product?.revievs?.reduce((acc: number, item: any) => acc + item.rating, 0) / product?.revievs?.lenght
      ...
      <PageContainer>
        <div className="block md:flex gap-10 justify-center">
          ...
          <div className="w-full md:w-1/2 space-y-3">
            ...
            <div className="text-slate-500">{product?.describtion}</div>
            <div className="flex items-center gap-2">
              <div>STOK DURUMU:</div>
              {
                product.inStock ? <div className="text-green-500"> Urun Stokta Mevcut</div> : <div className="text-red-500">Urun Stokta Yok</div>
              }
            </div>
